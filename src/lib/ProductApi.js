import { defaultFetch, tokenFetch, dynamicFetch } from './fetchClient';

export async function getBestProduct() {
  const data = await defaultFetch('/products?limit=4&orderBy=favorite', {
    next: { revalidate: 180 },
  });
  return data.list.sort((a, b) => b.favoriteCount - a.favoriteCount).slice(0, 4);
}
...
export async function getProducts(orderBy = 'recent', keyword = '', page = 1) {
  ...
  if (orderBy === 'oldest') {
    ...
    return { list: allItems.slice(start, start + pageSize), totalCount: data.totalCount ?? 0 };
  }

  const apiOrderBy = orderBy === 'like' ? 'favorite' : orderBy;
  const params = new URLSearchParams({ orderBy: apiOrderBy, pageSize: String(pageSize), page: String(page) });
  if (keyword) params.set('keyword', keyword);
  const data = await defaultFetch(`/products?${params.toString()}`, { next: { revalidate: 180 } });
...
export const updateProduct = (productId, data) =>
  tokenFetch(`/products/${productId}`, { method: 'PATCH', body: JSON.stringify(data) });
...
export const createPcomment = (productId, content) =>
  tokenFetch(`/products/${productId}/comments`, { method: 'POST', body: JSON.stringify({ content }) });
